...
Concurrently, in 1979 Bajarne Stroustrup, the god father of \texttt{C++}, started the "C with classes"\link{http://www.stroustrup.com/bs_faq.html\#invention} which later becomes \texttt{C++}. Stroustrup added classes, derived classes, inlining, and default arguments to the \texttt{C} compiler~\cite{stroustrup1996history}. Note that the name \texttt{C++} means the increment of \texttt{C} using the increment operator \cpp{++}. The \texttt{C++} Programming Language, but not as an official standard was released in 1985~\cite{stroustrup2000c++}. In 1989, C++98~\cite{koenig1998standard}, in 1999 C++ 2.0, in 2003 C++03~\cite{c++2003iso}, in 2011 C++11~\cite{c++2011iso}, and in 2014 C++14~\cite{c++2014iso}. The latest standard is C++17~\cite{smith2017iso} and the upcoming one is C++20.
...
\index{access specifiers}
\index{structure! access specifiers}
\label{sec:access:specifiers}
%----------------------------------------------------------------------------------------
for all functions and members of a \cpp{struct} or \cpp{class} the accessibility can be restricted to following access specifiers\link{https://en.cppreference.com/w/cpp/language/access}:
\begin{itemize}
\item \cpp{public} - All members below this expression are accessible from outside \index{access specifiers!public}
\item \cpp{public} - All members below this expression are not accessible from outside\text{.}\index{access specifiers!private}
\end{itemize}
\vspace{0.25cm}
Listing~\ref{code:access:specifiers} shows the usage of the access specifiers. Since \cpp{double x} is declared as \cpp{private}, we can not access the struct member using \cpp{a.x} anymore, since we only allowed access within the definition of the \cpp{struct}. To access the \cpp{private} member, a so-called get method, see Line~11, and a do-called set method, see Line~9 is declared in the \cpp{public} scope of the \cpp{struct}.
...
\index{inheritance}
%----------------------------------------------------------------------------------------
In some cases we like to define a base class, \emph{e.g.}\ for a shape, see Listing~\ref{code:class:inheritance}. Here, we define the \cpp{class Shape} and the expression \cpp{abstract}\link{https://en.cppreference.com/w/cpp/language/abstract_class}\index{class!abstract} means that we can not generate instances of the class. This class will only be used to derive different shapes, like \cpp{class Rectangle} and \cpp{class Square}. To inherit all functions and class members to from the parent class to the child class, the expression \cpp{class Rectangle : public Shape}\link{https://en.cppreference.com/book/intro/inheritance} is used, see Line~19. Fir the usage of the access specifiers, we refer to Section~\ref{sec:access:specifiers}. Here, the new access specifier \cpp{protected} is introduced. We assume that each derived shape has a \cpp{width} and a \cpp{height}. To showcase the difference between \cpp{private} and \cpp{protected}, the \cpp{width} is declared with \cpp{private} access and the \cpp{height} is declared as \cpp{protected}, respectively. Note that both of these class members are inherit to the child class, however, both of them are accessed differently. Since the \cpp{width} in the parent class wad declared as \cpp{private}, we only can access this class member using the parent class. In Line~23, the constructor of the child class \cpp{Rectangle} is defined. To access the \cpp{private} class member we have to use \cpp{: Shape(width)} since only the parent class can access it. For the \cpp{protected} variable, we can write \cpp{height = height} since we can access this variable directly.
...
%----------------------------------------------------------------------------------------
\index{pointer!objects}
Listing~\ref{code:pointer:object} shows the definition of the \cpp{struct A}, for more details about \cpp{struct} we refer to Section~\ref{sec:struct}. In Line~12 we use a point \cpp{A* a} and assign the address of the \cpp{new A(10)} object to it. Note for accessing the variable \cpp{value} using the pointer, the expression \cpp{->}\link{https://en.cppreference.com/w/cpp/language/operator_member_access} is used. 
